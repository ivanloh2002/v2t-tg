...
**Пожайлутса**, прочитайте рдими полностью, чтобы избежать недопонимания. 
...
Здесь используется **faster_whisper** для расшифровки адуио. 
...
Также есть режим ращдельной отправки текста. Сначал отправляется сырой текст, а потом, когда готов уже обработанный, он заменяется тем, который был изначально. **Это никак не влияет на производительность**, но делат видимость, что бот рабоатет быстрее. 

В сумме, во время работы у меня занимает оыбчно (с моделью *qwen3.5-4b-q5* и *large-v3-turbo*) **4610 мб видео памяти**. Но, с учётом других приложений получается **6 гигов из 8**. 
...
Я использую ноутбук с RTX3070ti на 8 гигов VRAM. Он переводил сообщение **длиной в 7 минут за 44 секунды** (На модели *medium*) (я считал по логам aiograma). А на модели *large-v3-turbo* Это же гс у меня обработалось за **22 секунд**. Это, если что, время полного цикла, т.е расшифровка + доработка.
...
Также есть поддрежка *qwen3.5-2b-q5*, она гораздо быстрее, окло **160 токенов в секнду** по сравнению с *qwen3.5-4b-q4* (**82 токена в секунду**) и *qwen3.5-4b-q5*(**72 токена в секунду**), в итоге это расшифврка с *qwen2.5-2b-q5* на **27 процентво быстрее**. Но qwen на 2 миллирада параметров **гораздо менее качественная**, так что выбирайет сами.  
## **Структура проекта** ##
